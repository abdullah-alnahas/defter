...
- **Symmetric 3-column grid** (`Page.svelte`): equal-width left margin / body (capped at `--measure: 44rem`) / equal-width right margin (`--margin-col: 9rem`). Body is centred. Left margin column kept as visual balance for the right sidenote column even though nav no longer lives there. (Widened from 34rem/13rem on 2026-05-24 — margins felt too generous, body felt cramped at 19px.)
