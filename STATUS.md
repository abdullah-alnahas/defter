...
### Page closing block
- [ ] Bundle Quran font (`UthmanicHafs_V22.ttf` or `UthmanTN_v2-0.ttf`) — subset to closing-Ayah glyphs to keep payload tiny.
- [ ] `@font-face` with `font-display: swap`, preload the file used for the closing Ayah.
- [ ] Update `Page.svelte` closing slot: Ayah in Quran font + Ibrahimi salawat with سيدنا prefix (replace current bare salawat/hamd).
